...
const LIMITS = [
  ...
  ["It catches 18 of 23 real sandwiches, not 23 of 23.", "Requiring a victim in between took false positives on ordinary mainnet blocks from 35 to 0, and cost 5 detections. Those 5 are unrecoverable by a hook: afterSwap sees one swap and two storage words, not the whole block. The specific gap is when the run's opener is also the party in the middle — there is no third slot for them, so the exit reads as the victim closing their own position."],
      ...
      <Reveal>
        <section className="panel">
          ...
          <p className="sub">
            ...
            of 25 real mainnet sandwiches — and precision never had. Replayed against 117 real
            mainnet blocks containing trading and no sandwich,{" "}
            <code>BlockReversal</code> as shipped fired on <strong>35 of them</strong>. Around three
            fifths of everything it flagged was innocent, because "the pool reversed direction under
