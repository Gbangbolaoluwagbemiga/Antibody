...
contract CreatePoolAndAddLiquidityScript is BaseScript, LiquidityHelpers {
    ...
    // MUST be the dynamic-fee flag, not a static fee. AntibodyHook sets the LP fee per swap via
    // the beforeSwap override, and the PoolManager only honours that on a dynamic-fee pool — a
